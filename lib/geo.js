...
/* ── Reporting a misconfigured key ──
   Google does not reject the promise when a key is bad. It draws a grey box,
   logs to the console and calls window.gm_authFailure. Without this hook the
   four ways a fresh Cloud project is normally broken — billing off, wrong
   referrer, Places not enabled, key restricted to the wrong APIs — all look
   identical to "no results", which is the worst thing to hand someone who
   has just set the key up. */
...
function installAuthHook() {
  ...
  window.gm_authFailure = () => {
    const msg = 'Google rejected the key. Usual causes, in order: billing is '
      + 'not enabled on the Cloud project; this domain is not in the key\'s '
      + 'HTTP-referrer list; or the key is restricted to APIs that exclude '
      + 'Maps JavaScript. See LOCATIONS.md steps 3 and 4.';
    for (const fn of _authWatchers) { try { fn(msg); } catch { /* ignore */ } }
  };
}
...
export async function suggest(maps, input, bias) {
  ...
  // Present only when Places API (New) is enabled on the project AND the key
  // is allowed to call it. Checked explicitly so the owner gets told which
  // box to tick instead of an address field that silently never matches.
  if (!maps.places || !maps.places.AutocompleteSuggestion) {
    throw new Error('Places API (New) is not available for this key. Enable it in '
      + 'the Cloud console and include it in the key\'s API restrictions '
      + '(LOCATIONS.md steps 3 and 4).');
  }
